#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Arc III pass 2: exact contexts, cross-line quote state, single-quote
classification, lowercase proper nouns, typo sweep, meta-marker sweep.
Adapted from QA/lint_pass2.py. Read-only."""
import re, os, io, sys
...
out = open(os.path.join(here, "arc3_lint_pass2.txt"), "w", encoding="utf-8")
...
for fname in FILES:
    ...
    names = ["wengari","veylar","veylara","pyrina","pyrinae","styx","threx","lightbringer",
             "chi'thak","humman","hummans","human","humans","bright paw","shadow paw",
             "stripe paw","motted paw","white dawn","black fire","kyrie tree","kyre tree",
             "bright paws","shadow paws","stripe paws","motted paws","seris","kareth",
             "fire feet","arena"]
    counts = {}
    for i, rline in enumerate(raw, 1):
        t = strip_tags(rline)
    ...
    P("--- TYPO SWEEP ---")
    typos = [("gratious", r"\bgratious\b"), ("payed", r"\bpayed\b"),
             ("producy", r"\bproducy\b"), ("assasins", r"\bassasins?\b"),
             ("appropiate", r"\bappropiate\b"), ("handt", r"\bhandt\b"),
             ("wanning", r"\bwanning\b"), ("kyrie", r"\bkyrie\b"),
             ("dessert sun", r"\bdessert sun\b"), ("three thousands", r"\bthree thousands\b"),
             ("Raise and eyebrow", r"Raise and eyebrow"), ("my father son", r"\bmy father son\b"),
             ("t'vat", r"\bt'vat\b", re.I), ("stripes paw", r"\bstripes paw\b", re.I),
             ("hell (he'll)", r"\bhell\b"), ("whitedawns", r"\bwhitedawns\b"),
             ("father say", r"\bfather say\b"), ("father did", r"\bfather always did\b")]
    for entry in typos:
    ...
    markers = [r"^\s*Let me\b", r"^Here is the\b", r"^Yes\.\s", r"\bIt also fits the\b",
               ...
               r"\bCorrected\b", r"\bVersion A\b", r"\bVersion B\b", r"\bv1\b", r"\bv2\b",
               r"\bLore Confirmed\b", r"\bAuthor\b.*\bnote\b", r"\bcraft note\b",
               r"\bI'll take the seed\b", r"\bLet me rewrite\b", r"\bLet me narrate\b",
               r"\bHere is the correction\b", r"\bRewrite\b"]
    for i, rline in enumerate(raw, 1):
        t = strip_tags(rline)
        for mk in markers:
