...
# -*- coding: utf-8 -*-
"""
Ethra chapter lint — Arc V proactive audit (adapted from QA/lint_chapters.py).
Read-only. Scans content/story/chapters/chapter-arc5-NN.md (22 files).
Outputs QA/arc5_tooling/a5_lint_results.json + prints readable summary.
"""
...
for fname in FILES:
    ...
    for i, raw in enumerate(raw_lines, 1):
        ...
        low = txt.lower()
        for key, pat in [
            ("bright_mane_two_words", "bright mane"),
            ("brightmane_one_word", "brightmane"),
            ("draft_tores", "tores"),
            ("draft_visibily", "visibily"),
            ("draft_trough", "trough"),
            ("draft_yells", " yells "),
            ("draft_says", " ajani says "),
            ("fire_the_ray", "fire the ray"),
            ("it_s_body", "it's body"),
        ]:
            if pat in low:
...
with open(os.path.join(OUTDIR, "a5_lint_results.json"), "w", encoding="utf-8") as fh:
